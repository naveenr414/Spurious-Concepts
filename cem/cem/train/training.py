...
def construct_model(
    ...
    if config["architecture"] in ["ConceptEmbeddingModel", "MixtureEmbModel"]:
        ...
        extra_params = {
            ...
            "related_concepts": config.get("related_concepts",None),
            "limit": config.get("limit",None),
            "vae_model": config.get("vae_model",None),
            "adversarial_intervention": adversarial_intervention,
            "training_intervention_prob": config.get(
                'training_intervention_prob',
                0.0,
            ),
            "stratified_model": config.get("stratified_model",False),
            "concept_pair_loss_weight": config.get('concept_pair_loss_weight',0.0),
